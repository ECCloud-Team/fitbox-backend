...
const { broadcast } = require("../utils/websocket"); // Import broadcast function
...
router.post("/", auth, upload.none(), async (req, res) => {
  ...
  try {
    ...
    await folder.save();
    // Broadcast the new folder to all clients
    log(
      `Folder created: ${folder.name} by user ${userId} in parent folder ${
        parent_id || "root"
      }`,
      "POST",
      userId
    );
    broadcast({ type: "new-folder", data: folder });
    res.json(folder);
  } catch (err) {
...
router.put("/rename/:id", auth, async (req, res) => {
  ...
  try {
    ...
    // Broadcast the folder renaming to all clients
    broadcast({ type: "rename-folder", data: folder });
    res.json(folder);
  } catch (err) {
...
router.delete("/:id", auth, async (req, res) => {
  ...
  try {
    ...
    log(
      `Folder and its contents deleted: ${folder.name} by user ${userId}`,
      "DELETE",
      userId
    );
    // Broadcast the folder delete to all clients
    broadcast({ type: "delete-folder", data: { id: req.params.id } });
